...
var Config = class {
  constructor() {
    ...
    this.icons = true;
    this.showKinds = [];
    this.onlyNearestKinds = [];
    this.maxItems = 8;
    this.maxItemsIndicator = "\u2026";
    this.setConfiguration();
  }
  setConfiguration() {
    this.labels = import_coc.workspace.getConfiguration("suggest").get("completionItemKindLabels", {});
    const config2 = import_coc.workspace.getConfiguration("symbol-line");
    this.default_ = config2.get("default");
    this.separator = config2.get("separator");
    this.onlyNearestKinds = config2.get("onlyNearestKinds");
    this.maxItems = config2.get("maxItems");
    this.maxItemsIndicator = config2.get("maxItemsIndicator");
    const checkBoolean = (key) => config2.get(key);
...
function getSymbolKind(kind) {
  switch (kind) {
    case import_vscode_languageserver_protocol.SymbolKind.File:
      return "File";
    case import_vscode_languageserver_protocol.SymbolKind.Module:
      return "Module";
    case import_vscode_languageserver_protocol.SymbolKind.Namespace:
      return "Namespace";
    case import_vscode_languageserver_protocol.SymbolKind.Package:
      return "Package";
    case import_vscode_languageserver_protocol.SymbolKind.Class:
      return "Class";
    case import_vscode_languageserver_protocol.SymbolKind.Method:
      return "Method";
    case import_vscode_languageserver_protocol.SymbolKind.Property:
      return "Property";
    case import_vscode_languageserver_protocol.SymbolKind.Field:
      return "Field";
    case import_vscode_languageserver_protocol.SymbolKind.Constructor:
      return "Constructor";
    case import_vscode_languageserver_protocol.SymbolKind.Enum:
      return "Enum";
    case import_vscode_languageserver_protocol.SymbolKind.Interface:
      return "Interface";
    case import_vscode_languageserver_protocol.SymbolKind.Function:
      return "Function";
    case import_vscode_languageserver_protocol.SymbolKind.Variable:
      return "Variable";
    case import_vscode_languageserver_protocol.SymbolKind.Constant:
      return "Constant";
    case import_vscode_languageserver_protocol.SymbolKind.String:
      return "String";
    case import_vscode_languageserver_protocol.SymbolKind.Number:
      return "Number";
    case import_vscode_languageserver_protocol.SymbolKind.Boolean:
      return "Boolean";
    case import_vscode_languageserver_protocol.SymbolKind.Array:
      return "Array";
    case import_vscode_languageserver_protocol.SymbolKind.Object:
      return "Object";
    case import_vscode_languageserver_protocol.SymbolKind.Key:
      return "Key";
    case import_vscode_languageserver_protocol.SymbolKind.Null:
      return "Null";
    case import_vscode_languageserver_protocol.SymbolKind.EnumMember:
      return "EnumMember";
    case import_vscode_languageserver_protocol.SymbolKind.Struct:
      return "Struct";
    case import_vscode_languageserver_protocol.SymbolKind.Event:
      return "Event";
    case import_vscode_languageserver_protocol.SymbolKind.Operator:
      return "Operator";
    case import_vscode_languageserver_protocol.SymbolKind.TypeParameter:
      return "TypeParameter";
    default:
      return "Unknown";
  }
}
function isSymbols(symbols) {
...
// src/index.ts
var DocumentSymbolLine = class {
  constructor() {
    this.disposables = [];
    this.state = {};
    import_coc3.workspace.onDidChangeConfiguration(() => config.setConfiguration(), this, this.disposables);
  }
  dispose() {
  ...
  async getDocumentSymbols(bufnr) {
    ...
    if (!doc || !doc.attached)
      return;
    if (!import_coc3.languages.hasProvider("documentSymbol", doc.textDocument))
      return;
    ...
    const symbols = await import_coc3.languages.getDocumentSymbol(doc.textDocument, token);
    if (!symbols || !symbols.length) {
      return;
    }
    return convertSymbols(symbols);
  }
  async getSymbols(bufnr) {
    let symbols = await this.getDocumentSymbols(bufnr);
    if (!symbols || symbols.length === 0)
      return;
    const position = await import_coc3.window.getCursorPosition();
    const {showKinds} = config;
    symbols = symbols.filter((s) => s.range && showKinds.includes(s.kind) && positionInRange(position, s.range) == 0);
    ...
    symbols.forEach((symbol) => {
      ...
      } else if ((newSymbols[lastIdx].level != void 0 && symbol.level != void 0 ? newSymbols[lastIdx].level == symbol.level : false) || onlyNearestKinds.includes(symbol.kind) && newSymbols[lastIdx].kind == symbol.kind) {
  ...
  async refresh(bufnr) {
    ...
    symbols.forEach((symbol, index) => {
      ...
      if (!addedEllipsis) {
        ...
        if (currentItems == maxItems && leftItems > 0) {
          addedEllipsis = true;
          line = fullLine + `%#CocSymbolLine#${sep}%#CocSymbolLineEllipsis#%0@coc_symbol_line#expand@${maxItemsIndicator}%X`;
        } else {
          line = fullLine;
        }
      }
    ...
    if (!line) {
      if (default_ === "%f")
        line = `%#CocSymbolLineFile#${icons ? labels.file || "\u25A4" : ""} %#CocSymbolLine#%f`;
      if (default_.length > 0)
        line = "%#CocSymbolLine#" + line;
      if (line)
        fullLine = line;
    }
    const buffer = import_coc3.workspace.getDocument(bufnr).buffer;
    try {
  ...
  async clean() {
    Object.keys(this.state).forEach(async (bufnr) => {
      const exists = await import_coc3.workspace.nvim.call("bufexists", Number(bufnr));
      if (exists != 1) {
        delete this.state[bufnr];
      }
    });
  }
  async click(id, mouse) {
    const items = id.toString().split("989").map((v) => Number.parseInt(v));
    const bufnr = items[0];
    const index = items[1];
    ...
    if (mouse == "l") {
      ...
      import_coc3.workspace.selectRange(symbol.range);
    }
  }
};
async function activate(context) {
  ...
  import_coc3.events.on("CursorHold", async (bufnr) => {
  ...
  const timerRedraw = setInterval(() => {
    import_coc3.workspace.nvim.command("redrawtabline", true);
  }, 500);
  ...
  }, import_coc3.commands.registerCommand("symbol-line._click", async (id, mouse) => {
    await symbolLine.click(id, mouse);
  }, null, true), symbolLine);
}
